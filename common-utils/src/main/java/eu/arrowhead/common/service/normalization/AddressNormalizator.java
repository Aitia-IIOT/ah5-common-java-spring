	...
	private String normalizeMAC(final String candidate) {
		...
		if (candidate.contains(DOT)) {
			...
			for (int i = 0; i < groups.length; ++i) {
				groups[i] = candidate.substring(startIdx, startIdx + 2);
				startIdx = startIdx + 2;
			}
		}

		final Stream<String> stream = Arrays.asList(groups).stream();
		if (!Utilities.isEmpty(stream.filter(p -> p.length() != 2).collect(Collectors.toList()))) {
			return candidate; // not MAC
		}

		return stream.collect(Collectors.joining(COLON));
	}
